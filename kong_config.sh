...
curl -i -X POST http://localhost:8001/upstreams \
  ...
  -d 'name=navi-api&healthchecks.active.http_path=/&healthchecks.active.timeout=5&healthchecks.active.concurrency=10&healthchecks.active.healthy.interval=30&healthchecks.active.unhealthy.interval=30&healthchecks.active.healthy.successes=10&healthchecks.active.unhealthy.tcp_failures=5&healthchecks.active.unhealthy.timeouts=10&healthchecks.active.unhealthy.http_failures=5'

curl -i -X POST http://localhost:8001/upstreams/navi-api/targets \
  -H 'Content-Type: application/x-www-form-urlencoded' \
  -d 'target=navi-api-1:10010&weight=100'

curl -i -X POST http://localhost:8001/upstreams/navi-api/targets \
  -H 'Content-Type: application/x-www-form-urlencoded' \
  -d 'target=navi-api-2:10010&weight=50'

curl -i -X POST http://localhost:8001/services/ \
  -d 'name=navi-api' \
  -d 'url=http://navi-api/'

curl -X POST http://localhost:8001/services/navi-api/routes \
  -H 'Content-Type: application/x-www-form-urlencoded' \
  -d 'hosts[]=localhost'
